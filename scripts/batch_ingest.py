...
用法:
    python scripts/batch_ingest.py --source "/path/to/your/pdf/folder" --limit 15
...
USER_RESEARCH_CONTEXT = (
    "用户是材料/催化方向的研究者, 主要用 VASP 做第一性原理计算, 聚焦酸性 OER 电催化剂"
    "(Ir/Co/Ru 氧化物及掺杂体系)的溶出、烧结等失活机理, 也涉及 PEM 水电解/燃料电池的"
    "膜电极界面。文献库不止收录电催化, 也有跨领域文献。"
)
...
{{
  ...
  "tags": ["受控大类,1~4个。优先从这个词表选: {', '.join(TAG_VOCAB)}；论文所属学科词表里都没有贴切的,才新增1个简短标签,不要为了凑数硬套电催化标签"],
  "keywords": ["5~10个自由细粒度关键词, 是这篇论文最核心的检索抓手。要求: (1)覆盖四类维度各取最具体的: 【材料/体系】具体到组分与形貌(如 'Co-doped RuO2 nanosheet' 而非泛泛的 'catalyst'), 【机理/现象】具体到过程(如 'lattice oxygen mechanism'/'proton shuttling' 而非泛泛的 'mechanism'), 【方法/技术】文中真正起关键作用的方法名(如 'operando XAS'/'DFT+U'), 【应用/性能指标】如 'acidic OER'/'PEMWE durability'; (2)英文为主, 用领域内规范写法(名词短语, 不要整句), 化学式/缩写按标准大小写; (3)优先复用领域内已通行的术语措辞, 同义概念只保留一种写法(DFT 与 first-principles 只留 DFT), 避免制造近义词变体污染关键词索引; (4)只写论文确实做了/讨论了的, 边缘一提的不算核心, 宁缺毋滥不凑数不瞎编"],
  "类型": "从 [实验, 计算, 计算+实验, 建模, 综述] 里选一个, 判定标准: 【计算】特指材料/原子尺度的第一性原理或分子模拟——DFT/DFT+U/AIMD/MD/相场/蒙特卡洛/机器学习势等直接算材料的结构-能量-性质, 只有全篇是这类纯理论材料计算、无湿实验时才填'计算'; 【实验】有湿法合成/电化学测试/器件测试/谱学表征等实体实验; 【计算+实验】材料计算和实验兼有; 【建模】非材料原子尺度的系统级/宏观建模——如能源-经济集成评估模型(IAM)、技术经济分析、工艺流程模拟、纯数据统计/机器学习建模(不含材料原子计算)等, 这类论文虽然也'算', 但不是材料计算, 不要误填'计算'; 【综述】综述/展望类文章。",
  "方法关键词": "计算类写泛函/AIMD等方法学关键词; 实验类写主要表征手段(仅写文中能确定的); 建模类写模型名/情景设计/优化方法",
  "表征方法": ["标准化表征技术列表, 供按手段检索(如'哪些文献用了 operando XAS')。规则: (1)只收录论文里实际用来获取数据的表征/分析技术, 一次通读全文(含方法节/图注/SI 描述)把真正用到的都收全, 不要只凭摘要漏掉方法节才提到的技术; (2)命名标准化: 纯缩写(DEMS/XPS/TEM/XRD/NMR/SEM/AFM/EELS/EXAFS/XANES/ICP-MS 等)直接大写书写; 同一技术只用一种规范写法(HAADF-STEM 不写成 'STEM-HAADF'); (3)带'原位/工况/非原位'前缀的, 前缀与技术名之间用空格: 中文'原位'→'in situ'、'工况下'→'operando'、'非原位/离线'→'ex situ'(写 'in situ Raman'/'operando XRD'/'ex situ XPS', 不写 'insitu-Raman'/'InsituXRD'/粘连或连字符形式); (4)区分表征技术与理论方法——DFT/AIMD 属'方法关键词'不进这个字段; (5)原文含糊(只说'多种谱学表征'没点名具体技术)就不编, 纯计算/建模论文或全文未点名任何具体技术则填 []"],
...
def group_and_pair(source: Path):
    ...
    all_files = [
        ...
        and root_resolved not in p.resolve().parents  # 防止 source 覆盖到本库自己(Claude_reference/papers 等), 重复入库自己的产出
...
def run_one_batch(client, args, state, dois, citekeys, title_doi_map, main_to_si, all_mains, unmatched_si, weak_si=None) -> int:
    ...
    todo = [p for p in all_mains if str(p) not in state][: args.limit]
    ...
    for pdf_path in todo:
        ...
        try:
            ...
            if len(text.strip()) < 50:
                ...
                state[str(pdf_path)] = {"status": "error", "error": "PDF 抽不出文字(可能是扫描件)"}
                continue
            ...
            # Crossref 现在是 citekey 三要素(年份/期刊/DOI)的权威源, 不再只是 DOI 缺失时的兜底:
            # 只要 DeepSeek 抽出了标题就正查一次, 命中则覆盖 doi/title/journal/year/authors_full,
            # AI 抽取只负责 Crossref 给不了的字段(表征方法/摘要要点/笔记正文等)。查不到就原样保留
            # DeepSeek 抽取结果, 见 lookup_metadata_via_crossref 注释。
            ...
            state[str(pdf_path)] = {"status": "error", "error": str(e)}
...
def main():
    sys.stdout.reconfigure(encoding="utf-8")  # 避免 Windows GBK 控制台把中文输出打乱/崩溃
