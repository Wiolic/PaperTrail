...
{{
  ...
  "keywords": ["5~10个自由细粒度关键词(英文为主,具体材料/机理/方法名), 不确定就少给几个, 不要瞎编"],
  "类型": "计算 / 实验 / 计算+实验 / 综述 中选一个",
  "方法关键词": "计算类写泛函/AIMD等方法学关键词; 实验类写主要表征手段(仅写文中能确定的)",
  "表征方法": ["标准化表征技术列表, 供检索用。纯缩写(DEMS/XPS/TEM/XRD/NMR/SEM/AFM/EELS等)直接大写书写; 带'原位/操作/非原位'前缀的技术, 前缀和技术名之间用空格(写 'in situ Raman'/'operando XRD'/'ex situ XPS', 不要写 'insitu-Raman'/'InsituXRD'/粘连或连字符形式); 中文'原位'统一译成'in situ','工况下'用'operando','非原位/离线'用'ex situ'; 只列文中明确点名的具体技术, 原文含糊(如只说'多种谱学表征'没写具体技术)就不要编, 纯计算论文或全文未点名任何具体技术则填 []"],
