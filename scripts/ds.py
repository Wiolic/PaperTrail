...
def cmd_pdf_meta(args):
    ...
    client = get_client()
    result = call(client, args.model, b.build_extract_system_prompt(), text, temperature=0, json_mode=True,
                  reasoning_effort=args.reasoning_effort)
    json.loads(result)
...
def main():
    ...
    p.add_argument("pdf")
    # pdf-meta 同时产出元数据(爬取范畴)和七节精读笔记正文(读者真正会看的总结输出),
    # 默认用 pro 档模型保证笔记正文质量, 不用全局默认的 flash(那个是给纯抽取/零碎任务用的)。
