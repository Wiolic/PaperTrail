...
用法:
  1. 把候选列表写成 JSON(见 --candidates)：
     [
       ...
       {"raw_text": "从网页复制的一段没有清晰 DOI 的文字...", "link": "https://...", "category": "示例分类A"}
     ]
...
def deepseek_extract(raw_text: str, link: str) -> dict:
    ...
    result = ds.call(client, "deepseek-chat", system, raw_text, temperature=0, json_mode=True)
...
# [领域定制] 下面这套分类规则是电催化文献库的示例(按 Ir/Ru 化学元素分类), 换成你自己领域的
# 分类维度即可(比如按细分技术路线/材料体系/研究对象分类), classify_category() 的整体思路
# (关键词/token命中判断, 命中多个就是"mixed"类, 都不命中就是Unclear)可以照搬。
