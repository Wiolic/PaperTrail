...
`OER` `HER` `DFT` `原位表征` `综述` `方法学` `电池` `2D材料` `MXene` `钙钛矿` `结构生物学` `高分子` `单原子催化` `拉曼光谱` `半导体光电` ...

论文明显不属于已有分类时才新增标签，不要为了凑数把无关论文硬塞进已有标签。

**大类标签和具体材料子类标签不互斥，都要打**：如果论文的具体研究对象有一个公认的专有名称（如 MXene、LDH、graphene oxide 等），在打大类标签（如 `2D材料`）的同时，必须把这个具体名称也作为独立标签加进去——不能只有一个模糊大类、看不出具体是哪种材料，否则同类论文之间按材料检索不到彼此。判断标准：这个材料名称在库里大概率会反复出现，就该给它一个可复用的专属标签。

**词表自动跟库内容同步**：`scripts/batch_ingest.py` 里的 `TAG_VOCAB` 只是一份种子表，真正喂给 LLM 的词表由 `load_live_tag_vocab()` 每次动态生成——扫一遍 `notes/*.md` 的 `tags` 字段，把库里真实出现过的所有标签和种子表取并集。这样任何一次新造的具体标签（哪怕只被用过一次）从下一篇论文起就会自动被后续入库看到、被优先复用，不需要人工把新词手动加回代码。`EXTRACT_SYSTEM_PROMPT` 相应地从模块级常量改成了函数 `build_extract_system_prompt()`（每次调用都重新生成），`ds.py`/`regenerate_notes.py` 里引用它的地方也要调用这个函数而不是旧的常量名。
