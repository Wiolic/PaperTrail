...
一个用 LLM 驱动的**文献精读笔记库框架**：PDF 入库、结构化笔记生成、标签/关键词检索、主题综述、逐句核实引文、去重体检，全部脚本化、可断点续跑。
...
把 `scripts/ds.py` 换成你自己的 API/模型时，把这两个单价换成你实际用的定价页数字即可。像"引文献"逐句核实、"扩充/查新"候选抽取这类功能，单次调用的 token 量级比整篇入库还小（只处理摘要片段或搜索结果文字），成本可以忽略不计。
...
- **引文核实分方向判断，不是关键词命中就算**：`find_citations.py` 对每篇候选给 support/contradict/unclear 判断而不是笼统"相关"，见上方"重点功能"。
